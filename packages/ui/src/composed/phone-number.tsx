...
import { copyTextToClipboardSync } from "../lib/clipboard.js";
import {
  formatPhoneCopy,
  formatPhoneDisplay,
  formatPhoneSmsHref,
...
const defaultPhoneNumberLabels: PhoneNumberLabels = {
  menu: "Phone actions",
  call: "Call",
  copy: "Copy number",
  copied: "Copied",
...
export type PhoneNumberProps = {
  ...
  phone?: string;
  /** Backwards compatibility alias for phone */
  value?: string;
...
export function PhoneNumber({
  ...
  const resolvedLabels = labels ?? defaultPhoneNumberLabels;
  const valid = isValidIranMobile(rawPhone);
  const display = useMemo(() => formatPhoneDisplay(rawPhone), [rawPhone]);
  const copyValue = useMemo(() => formatPhoneCopy(rawPhone), [rawPhone]);
  const telHref = useMemo(() => formatPhoneTelHref(rawPhone), [rawPhone]);
  const smsHref = useMemo(() => formatPhoneSmsHref(rawPhone), [rawPhone]);
  ...
  if (variant === "copy") {
    return (
      <button
        ...
        aria-label={copied ? resolvedLabels.copied : resolvedLabels.copy}
      >
        <PhoneFigure display={display} copyValue={copyValue} />
        <PhoneIconAffordance
      ...
      >
        ...
        {showCopy ? (
          <DropdownMenuItem
            className="gap-2"
            onSelect={(event) => {
              // Keep menu gesture; sync copy must run in this turn (HTTP).
          ...
          >
            ...
            {copied ? resolvedLabels.copied : resolvedLabels.copy}
          </DropdownMenuItem>
        ) : null}
      </DropdownMenuContent>
    </DropdownMenu>
