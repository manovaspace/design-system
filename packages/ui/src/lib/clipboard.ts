 ...
 *
 * On insecure HTTP (`http://demo…`), `navigator.clipboard` is unavailable or
 * rejects. Menu items also steal focus, so a hidden-textarea `execCommand`
 * often returns false. Listening for the `copy` event and setting
 * `clipboardData` there is reliable inside the same user-gesture tick.
 */
